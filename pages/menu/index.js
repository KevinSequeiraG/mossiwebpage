...
const Menu = () => {
  ...
  return (
    <div
      className={`relative ${
        showNewCatModal || showUpdateModal
    ...
    >
      <NavBar />
      <div
        className={`cardsContainer w-full h-full ${
          loggedUser.name ? "pt-20" : "pt-32"
        } pb-[10rem] bgMain min-h-screen`}
      >
        <div className="text-right w-11/12 mr-auto">
          {showMaintenance ? (
            <button
              className="text-[1rem] mt-2 lg:mt-8 px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-700 mb-10 mr-2"
              onClick={() => {
          ...
          {loggedUser.name ? (
            <button
              className="text-[1rem] mt-2 lg:mt-8 px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-700 mb-10"
              onClick={() => {
        ...
        <div className="w-11/12 h-11/12 top-40 inset-x-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-36 mx-auto justify-items-center gap-y-16">
