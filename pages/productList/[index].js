      ...
      <div className={`cardsContainer w-full h-full pt-20 pb-36 relative bgMain min-h-screen`}>
        <div className="text-left w-10/12 mx-auto mt-8 lg:mt-16">
          <div className="relative">
            ...
            {showMaintenance ?
              <button className="text-[1rem] absolute right-28 top-0 px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-700 mb-10 mr-2" onClick={() => {setShowNewProductModal(true)}}>
                Crear nuevo producto
              </button> : null}
            {loggedUser.name ?
              <button className="text-[1rem] absolute right-0 top-0 px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-700 mb-10" onClick={() => { setShowMaintence(!showMaintenance) }}>
        ...
        <div className="w-10/12 h-11/12 top-40 inset-x-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 mx-auto justify-items-center gap-y-12">
          {productData.map((data, i) => {
            return <ProductCard getProductData={()=>getProductData()} showMaintenance={showMaintenance} data={data} getProductData={getProductData} />;
          })}
        </div>
        <div className="absolute w-full bottom-[75px]">
          <Footer />
        </div>
        {showNewProductModal? <div className="w-full h-screen bg-black bg-opacity-50 absolute top-0 z-[999] fixed">
          <NewProductModal closeModal={()=>setShowNewProductModal(false)} getProductData={()=>getProductData()} />
        </div> : null}
      </div>
    </>
  );
