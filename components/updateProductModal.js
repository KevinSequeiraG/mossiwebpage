...
const UpdateProductModal = (props) => {
    ...
    const product = (data) => {
        ...
        updateDoc(productToEdit, {
            ...
            name: data.name,
            price: data.price,
            productImgUrl: `${data.productImgUrl != undefined ? data.productImgUrl : ""
                }`,
        })
            .then(() => {
                props.closeModal()
        ...
        <div className="w-[40%] h-auto bg-gray-800 border-2 border-gray-300 absolute top-1/2 left-1/2 z-[1000] translate-x-[-50%] translate-y-[-50%] rounded-lg py-5">
            ...
            <div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-4/5 mx-auto mt-8">
                    ...
                    <input className="text-[1rem] mt-2 lg:mt-8 px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-700 mr-2" type="submit" value={"Actualizar producto"} />
                </form>
            </div>
        </div>
    )
